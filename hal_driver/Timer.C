...
static timer_cb_t timer0_cb = NULL;
...
void timer0_register_cb(timer_cb_t cb)
{
    timer0_cb = cb;
}
...
{
    // T2MOD &= ~bT0_CLK: 选择标准时钟 Fsys/12
    mTimer0Clk12DivFsys();  // 12MHz/12 = 1MHz = 1us 每计数
    ...
    // 计算10ms所需计数值:
    // 10ms = 10000us
    // 所以计数值 = 65536 - 10000 = 0xD8F0
    mTimer_x_SetData(0,0xD8F0);  // 修改计数值为0xD8F0
...
void mTimer0Interrupt(void) interrupt INT_NO_TMR0 using 1
{
    ...
    {
        if (++ g_cleanmatch_timer >= 220)	// 1100
        {
            WL_LEARN_PIN = 1;
